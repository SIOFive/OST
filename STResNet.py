...
def _residual_unit(nb_filter, filter_size, init_subsample=(1, 1)):
    def f(input):
        residual = _bn_relu_conv(nb_filter, filter_size, filter_size)(input)
        residual = _bn_relu_conv(nb_filter, filter_size, filter_size)(residual)
...
def stresnet(c_conf=(10, 1, 52, 52), nb_residual_unit = 3, filter=64, filter_size=3):
    ...
    if c_conf is not None:
        ...
        # Conv1
        conv1 = Convolution2D(
            nb_filter=filter, nb_row=filter_size, nb_col=filter_size, border_mode="same")(input)
        ...
        conv2 = Convolution2D(
            nb_filter=nb_channel, nb_row=filter_size, nb_col=filter_size, border_mode="same")(activation)
